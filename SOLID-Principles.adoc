...
= Following the SOLID Design Principles Makes Test Driven Development Faster and Easier
...
One of the key detriments to going fast is having an overly large test suite where and a non-optimized application design.
The SOLID principles, helps you to build a well designed app that.  From a TDD perspective, you can design your tests in a way where
the complete behavior of the app is captured and tested, while minimizing the number of tests as well as redundant tests.
